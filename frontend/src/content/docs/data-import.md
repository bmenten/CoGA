...
Once imported, each sample's sequencing QC appears in the **Family members** table on the family page as a compact chip showing the QC verdict and mean depth. Hover for every recorded metric, the cut-offs it was judged against and which of them breached; click to open the pipeline's QC report in a separate tab.

The chip turns amber when a measurement crosses its warning cut-off and red when it crosses the error cut-off. Cut-offs are configured per assay profile under **Admin → Sequencing QC Thresholds** — no values ship by default, and a metric without a cut-off is reported as not assessed rather than as a pass.
