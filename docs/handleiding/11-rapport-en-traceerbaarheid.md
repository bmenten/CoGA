...
In dit hoofdstuk komt alles samen. U leest hoe een casus in CoGA wordt afgetekend ("sign-out") tot een **bevroren, geversioneerd en gehasht rapport-snapshot**, dat onlosmakelijk verbonden is met de software-, annotatie- en referentieversies die het produceerden. U ziet welke **gates** (drempels) vervuld moeten zijn voordat aftekenen mag — de classificatie-drift-check en de Sample-QC-erkenning — en hoe elke stap wordt vastgelegd in een **append-only, hash-geketende** audit- en sign-out-trail met externe **integriteitsankers**. De rode draad is de *volledige traceerbaarheidsketen*: van het ruwe importbestand met zijn hash, via het annotatie-manifest en de variant in ClickHouse, tot het ondertekende rapport en zijn extern verifieerbare anker. Dit is de kern van de explainability- en medico-legale boodschap voor het review board.
