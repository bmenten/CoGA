...
In dit hoofdstuk sluiten we de handleiding af met de modules die geen eigen hoofdstuk kregen, zodat de volledige codebase gedekt is. Je leert hoe curatoren de klinische-CNV-kennisbank doorbladeren, hoe herbruikbare genpanelen worden beheerd, wat elke adminfunctie precies doet en welke rechten daarvoor nodig zijn, hoe de in-app documentatie en de releasepagina werken, en hoe de UI-telemetrie (klik- en navigatiegedrag van gebruikers) in dezelfde duurzame audit-pijplijn terechtkomt als de HTTP-audittrail uit [hoofdstuk 11](11-rapport-en-traceerbaarheid.md). We eindigen met een dekkingschecklist die per top-level map aangeeft in welk hoofdstuk die aan bod komt.
